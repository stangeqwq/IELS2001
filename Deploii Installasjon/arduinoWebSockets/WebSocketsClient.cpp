...
void WebSocketsClient::loop(void) {
    ...
    if(!clientIsConnected(&_client)) {
        ...
        if(_client.isSSL) {
         ...
         // _client.ssl->setNoDelay(true); // disable Nagles algorithm (does not work for some reason here)
...
#error setCACert not implemented
            ...
            _client.tcp->setNoDelay(true); // disable Nagles algorithm (this is where you access the WiFiClient library)

        }
#else
        _client.tcp = new WEBSOCKETS_NETWORK_CLASS();

        _client.tcp->setNoDelay(true);  // disable Nagles algorithm
